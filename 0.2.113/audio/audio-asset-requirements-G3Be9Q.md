...
| `combat-token-pop` | Active | Fires for each little combat token when it appears and again when it pops away during the confrontation timeline. | `0.05s`–`0.25s` | No | Pleasant satisfying UI pop. | Keep it pleasing and punchy because the cue can overlap many times in quick succession. It should read as tactile feedback, not as a second clash layer. |
| `resource-flyout-land` | Active | Fires once per resource flyout sprite when it lands on the matching HUD chip. | `0.05s`–`0.25s` | No | Pleasant satisfying UI pop / ting. | Keep it readable when several resources land in fast succession; the amount gained should be legible from the number of landings you hear. |
...
| `combat-outcome-favorable` | Active | Fires when the confrontation result panel is revealed after a player-favorable exchange. | `0.2s`–`0.8s` | No | Dark-fantasy favorable battle resolve. | Should feel like a tense tactical win, not a full run-ending victory sting. The shipped file is a small repository-generated WAV built from layered tonal/percussive synthesis. |
| `combat-outcome-unfavorable` | Active | Fires when the confrontation result panel is revealed after a player-unfavorable exchange. | `0.2s`–`0.9s` | No | Dark-fantasy setback / dread resolve. | Keep it sharp and somber without sounding like total game over every time. The shipped file is a small repository-generated WAV built from layered tonal/percussive synthesis. |
...
- `combat-token-pop` now uses Kenney **Casino Audio** (`chips-collide-2.ogg`, CC0).
- `resource-flyout-land` currently reuses the same Kenney **Casino Audio**
  `chips-collide-2.ogg` source as `combat-token-pop` via a separate runtime alias.
- `combat-clash` now uses Kenney **Impact Sounds** (`impactMetal_heavy_001.ogg`, CC0).
- `combat-outcome-favorable` and `combat-outcome-unfavorable` are
  repository-generated 44.1 kHz mono WAV stings synthesized from layered tonal,
  percussive, and noise envelopes for this confrontation outcome pass.
...
- `ui-drop`, `action-start`, `action-complete`, `build-complete`,
  ...
  `combat-token-pop`, `resource-flyout-land`, `combat-clash`, `combat-outcome-favorable`,
  `combat-outcome-unfavorable`,
...
- `combat-popup-open` fires on confrontation popup mount, while
  `combat-card-slam`, `combat-clash`, and the confrontation outcome cues are
  scheduled from the popup's GSAP timeline rather than from model-level combat
  state transitions.
- The confrontation result-reveal beat classifies the live combat scene into a
  player-favorable or player-unfavorable outcome before selecting which cue to
  play.
...
- `resource-flyout-land` is also intentionally configured to overlap with
  itself and fires once per landed flyout sprite so the HUD arrival audio count
  matches the number of resources gained.
