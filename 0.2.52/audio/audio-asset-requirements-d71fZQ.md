...
| `pack-spawn-sting` | Bundled, runtime-disabled | Reserved for the threat-warning sting when a new roaming pack is spawned at the anger threshold. | `0.4s`–`0.9s` | No | Ominous warning sting, sharp attack, short decay. | Keep it readable over the HUD and toast layer. Good candidates: detuned bell hit, metallic scrape, low forest shriek, or short brass stab. Avoid long reverb tails. Mono is fine. |
...
- The runtime registers `pack-spawn-sting`, but the current controller does **not**
  invoke it yet.
