...
| `combat-popup-open` | Active | Fires when a new confrontation popup mounts for pending combat. | `0.1s`–`0.5s` | No | Tense popup / confrontation reveal cue. | Keep it short so repeated combats do not drag. The current shipped file is a temporary popup cue rather than a long sting. |
...
- `combat-popup-open` currently reuses the shipped `survivor-hut-reveal` clip.
- `combat-card-slam` currently reuses the shipped `action-start` clip.
- `combat-clash` currently reuses the shipped `fortify-absorb` clip.
- `combat-result-reveal` currently reuses the shipped `poi-reward-empty` clip.
...
- `ui-drop`, `action-start`, `action-complete`, `build-complete`,
  `fortify-absorb`, `combat-popup-open`, `combat-card-slam`, `combat-clash`,
  `combat-result-reveal`, `poi-landmark-claim`, `poi-reward-found`,
